document.addEventListener('DOMContentLoaded', () => {
    ...
    function calc(d) {
        ...
        let growth = 0;
        if (len >= 10) {
            const fAvg = avg(scrs.slice(0, 5));
            const lAvg = avg(scrs.slice(-5));
            growth = lAvg - fAvg;
        }
        return {
            ...d.user,
            games: len,
            avg: avg(scrs).toFixed(1),
    ...
    function render() {
        ...
        const fDat = dat
            .map(calc)
            .filter(u => u.fio.toLowerCase().includes(q) || u.nick.toLowerCase().includes(q));

        fDat.sort((a, b) => {
            let vA = a[srt.key];
            let vB = b[srt.key];
            if (typeof vA === 'string') {
                return vA.localeCompare(vB) * (srt.asc ? 1 : -1);
            }
            return (vA - vB) * (srt.asc ? 1 : -1);
        });

        uBody.innerHTML = '';
        fDat.forEach(u => {
            const row = document.createElement('tr');
            row.innerHTML = `<td>${u.fio}</td><td>${u.yob}</td><td>${u.nick}</td><td>${u.games}</td><td>${u.avg}</td><td>${u.growth > 0 ? '+' : ''}${u.growth}</td>`;
            row.addEventListener('click', () => renderChart(u));
